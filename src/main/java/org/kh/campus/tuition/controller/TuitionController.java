...
public class TuitionController {
	...
	public NexacroResult tuitionSave(@ParamDataSet(name = "in_tui") DataSet inTui) throws Exception {
		...
		for (int i = 0; i < inTui.getRowCount(); i++) {
			...
			if (rowType == DataSet.ROW_TYPE_UPDATED) {
				...
				String chk = "";
				Tuition tuition = new Tuition(tuitionNo, studentNo, tutionYear, tuitionTerm, studentName, tuitionFee,
						tuitionScholarship, tuitionTotal, tuitionStatus, chk, departmentName, studentGrade);
	...
	public NexacroResult printOne(@ParamDataSet(name = "searchAll") DataSet searchAll, HttpSession session)
			throws Exception {
		NexacroResult result = new NexacroResult();
		int nErrorCode = 0;
		String strErrorMsg = "START";
